...
RSpec.describe ReadPWX::Parser do
  ...
  describe "parsing" do
    ...
    it "creates a pwx instance" do
      expect(parsed.pwx).to be_kind_of(ReadPWX::PWX::PWX)
    end

    describe "the pwx instance" do
      let(:pwx) { parsed.pwx }

      it "has an array of workout instances" do
        expect(pwx.workouts).to be_kind_of(Array)
      end

      it "creates a workout instance" do
        expect(pwx.workouts.first).to be_kind_of(ReadPWX::PWX::Workout)
      end

      describe "the workout instance" do
        let(:workout) { pwx.workouts.first }
